...
class NotifyHelper {
  ...
  tz.TZDateTime _nextInstanceOfTenAM(int hour, int minutes,int remeind ,String repeat,String date) {
    final tz.TZDateTime now = tz.TZDateTime.now(tz.local);
    var formattedDate = DateFormat.yMd().parse(date);
    final tz.TZDateTime fd=tz.TZDateTime.from(formattedDate, tz.local);
    tz.TZDateTime scheduledDate = tz.TZDateTime(tz.local, fd.year, fd.month, fd.day, fd.hour, fd.minute);
    scheduledDate= afterRemind(remeind, scheduledDate);
  ...
  tz.TZDateTime afterRemind(int remeind, tz.TZDateTime scheduledDate) {
    if (remeind==5 ) {
 ...
 */
  ...
  void _configureSelectNotificationSubject() {
    selectNotificationSubject.stream.listen((String payload) async {
      ...
      await Get.to(() => NotificationScreen(payloud: payload,));
    });
  }
}
